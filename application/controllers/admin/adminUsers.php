...
class AdminUsers extends CI_Controller {
    ...
    public function index(){
        ...
        $totalRow = $this->logicalexpert_model->countRows('users','userType','admin');
        $perPage = $this->paginationInitialize('' , $totalRow);
        $offset = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        $data['userData'] = $this->logicalexpert_model->getAllData('users',array('userType' => 'admin'),'','',$perPage,$offset);
        // create pagination links
        $data['links']= $this->pagination->create_links();
        $data['pageLink'] = 'adminUsers';
        $data['heading'] = 'Admin Users';
        $data['search'] = 'userFName';
    ...
    public function ajaxCall(){
        ...
        $searchData = isset($postArray['userFName']) ? $postArray['userFName'] : $this->session->userdata('search');
        $this->session->set_userdata('search' , $searchData);
        $like = array('userFName' => $searchData);
        $totalRow = $this->logicalexpert_model->countRows('users','userType','admin' , $like );
        ...
        $data['userData'] = $this->logicalexpert_model->getAllData('users',array('userType' => 'admin'), '' , '', $perPage , $offset ,$sortData , '' , '' , $like );
        $data['links']= $this->pagination->create_links();
        $data['pageLink'] = 'adminUsers';
     ...
     */
    public function paginationInitialize($perPage = 2 , $totalRow){
        // pagination start
